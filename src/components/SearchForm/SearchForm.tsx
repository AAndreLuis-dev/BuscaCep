...
export const SearchForm: React.FC<SearchFormProps> = ({ onSearch, onAddressSearch, loading }) => {
        ...
        <form onSubmit={handleAddressSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <select
              ...
              onChange={(e) => setAddressQuery(prev => ({ ...prev, uf: e.target.value }))}
              className="px-4 py-3 bg-horizon-bg-secondary border border-horizon-contrast-light rounded-lg focus:ring-2 focus:ring-horizon-primary focus:border-transparent transition-all text-horizon-text"
              required
            >
            ...
            <input
              ...
              onChange={(e) => setAddressQuery(prev => ({ ...prev, city: e.target.value }))}
              placeholder="Cidade"
              className="px-4 py-3 bg-horizon-bg-secondary border border-horizon-contrast-light rounded-lg focus:ring-2 focus:ring-horizon-primary focus:border-transparent transition-all text-horizon-text placeholder-horizon-text-muted"
              required
            />
            <input
              type="text"
              value={addressQuery.street}
              onChange={(e) => setAddressQuery(prev => ({ ...prev, street: e.target.value }))}
          ...
          <button
            type="submit"
            disabled={loading || !addressQuery.uf || !addressQuery.city || addressQuery.street.length < 3}
